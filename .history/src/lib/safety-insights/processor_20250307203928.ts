...
export async function findOrGenerateTakeaways(
  ...
  try {
    ...
    // First, try to get safety metrics for this location
    const safetyMetrics = await getNearestDistrictSafetyMetrics(latitude, longitude);
    
    // Then get nearby insights
    const nearbyInsights = await findNearbyInsights(latitude, longitude, radius);
    
    // Generate takeaways using both data sources
    ...
    // Add the neutral_takeaway field if missing
    if (generatedTakeaways && !('neutral_takeaway' in generatedTakeaways)) {
      return {
        ...generatedTakeaways,
