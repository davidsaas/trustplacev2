...
import { findOrGenerateLocationTakeaways } from '@/lib/gemini-takeaways';
...
export async function findOrGenerateTakeaways(
  ...
  try {
    // Get nearby insights to provide content for takeaways generation
    const insights = await findNearbyInsights(latitude, longitude, radius);
    
    if (!insights || insights.length === 0) {
      console.log('No insights found for this location, returning null takeaways');
      return null;
    }
    
    // Use our new service to find or generate takeaways
    const takeaways = await findOrGenerateLocationTakeaways(latitude, longitude, radius, insights);
    
    // Map the new service's response to our expected format
    return {
      id: takeaways.id,
      latitude: takeaways.latitude,
      longitude: takeaways.longitude,
      radius: takeaways.radius,
      positive_takeaway: takeaways.positive_takeaway,
      negative_takeaway: takeaways.negative_takeaway,
      neutral_takeaway: null, // We're not using neutral takeaways anymore
      created_at: takeaways.created_at,
      expires_at: takeaways.expires_at
    };
  } catch (error) {
