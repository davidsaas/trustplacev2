...
async function generateTakeaways(insights: any[]): Promise<{ positive: string | null; negative: string | null; neutral: string | null } | null> {
  try {
    const prompt = `Analyze these safety-related insights about a neighborhood and generate specific, granular safety takeaways.
    
    Create two distinct lists:
    1. "What's Good" - Specific safety positives mentioned in the insights
    2. "Watch Out For" - Specific safety concerns or warnings mentioned in the insights
    
    Format each point as a separate sentence, not as bullet points or numbered lists.
    Make each point explicit, concrete and specific - avoid generic statements.
    Each point should be actionable and help someone understand the specific safety aspects of this area.
    If no safety positives or concerns are found, return null for that category.
    ...
    {
      "positive": "Streets are well-lit at night. Multiple police stations nearby provide quick response times. Local businesses have security cameras. Neighborhood watch program is active.",
      "negative": "Some reports of car break-ins during overnight hours. Side streets can be poorly lit. Weekend nights get noisy near bars on Main Street.",
      "neutral": null
    }`;
