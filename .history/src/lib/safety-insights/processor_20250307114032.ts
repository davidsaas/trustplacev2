...
import { generateSafetyTakeaways } from '@/lib/gemini-service';
...
export async function findNearbyInsights(latitude: number, longitude: number, radiusKm: number = 2) {
  ...
  console.log(`Looking for insights within ${radiusKm}km of [${latitude}, ${longitude}]`);
  
  // First try to get safety-related insights from the database
  ...
  if (safetyError) {
    console.error('Error fetching safety-related insights:', safetyError);
  } else {
    console.log(`Found ${safetyData?.length || 0} safety-related insights from database`);
  }
  
  // If we found safety insights in the database, use them
  if (safetyData && safetyData.length > 0) {
    return sortByDistance(safetyData, latitude, longitude);
  }
  
  // If no safety insights in database, try to get additional insights without the safety flag
  console.log('No safety-related insights found in database, trying additional query');
  ...
  if (additionalError) {
    console.error('Error fetching additional insights:', additionalError);
  } else {
    console.log(`Found ${additionalData?.length || 0} additional insights from database`);
  }
  
  if (additionalData && additionalData.length > 0) {
    return sortByDistance(additionalData, latitude, longitude);
  }
  
  // If still no insights, try loading from local JSON data
  console.log('No insights found in database, trying local JSON data');
  try {
    // Get city name from coordinates using a simple approximation
    let city = "Los Angeles";  // Default city
    
    // Define city boundaries - this is a simple approximation
    const cityBoundaries = {
      "Los Angeles": { minLat: 33.7, maxLat: 34.5, minLng: -118.6, maxLng: -118.1 },
      "San Francisco": { minLat: 37.7, maxLat: 37.9, minLng: -122.5, maxLng: -122.3 },
      "New York": { minLat: 40.5, maxLat: 40.9, minLng: -74.1, maxLng: -73.7 }
    };
    
    // Determine city from coordinates
    for (const [cityName, bounds] of Object.entries(cityBoundaries)) {
      if (
    ...
    // Try to load local JSON data for the city
    const localJsonPath = `./data/${city.toLowerCase().replace(/\s+/g, '_')}_safety.json`;
    console.log(`Attempting to load local JSON data from: ${localJsonPath}`);
    
    // Process the local data
    const localInsights = await processLocalJsonFile(localJsonPath, city);
    console.log(`Loaded ${localInsights.length} insights from local JSON`);
    
    // Filter insights to those near the specified location
    const nearbyLocalInsights = localInsights.filter(insight => {
      if (!insight.latitude || !insight.longitude) return false;
      
      const latDiff = Math.abs(insight.latitude - latitude);
      const lngDiff = Math.abs(insight.longitude - longitude);
      
      return latDiff <= radiusDegrees && lngDiff <= radiusDegrees;
    });
    
    console.log(`Found ${nearbyLocalInsights.length} nearby insights from local JSON`);
    
    if (nearbyLocalInsights.length > 0) {
      // Mark all local insights as safety-related for processing
      const enhancedInsights = nearbyLocalInsights.map(insight => ({
        ...insight,
        is_safety_related: true,
        safety_score: insight.safety_score || 0.7  // Default safety score if not present
      }));
      
      return sortByDistance(enhancedInsights, latitude, longitude);
    }
    
    // If no nearby local insights found, use all local insights as a last resort
    if (localInsights.length > 0) {
      // Mark all local insights as safety-related for processing
      const enhancedInsights = localInsights.map(insight => ({
        ...insight,
        is_safety_related: true,
        safety_score: insight.safety_score || 0.7  // Default safety score if not present
      }));
      
      // Sort by distance (even though they might not be very close)
      return sortByDistance(enhancedInsights, latitude, longitude);
    }
  } catch (localDataError) {
    console.error('Error processing local JSON data:', localDataError);
  }
  
  // Use hardcoded sample data as a last resort
  console.log('No insights found from any source, using hardcoded sample data');
  return generateSampleSafetyInsights(latitude, longitude);
}

/**
 * Sort insights by distance from a point
 */
function sortByDistance(insights: any[], latitude: number, longitude: number) {
...
/**
 * Generate sample safety insights as a last resort when no data is available
 */
function generateSampleSafetyInsights(latitude: number, longitude: number) {
  const insights = [
    {
      comment_id: 'sample-positive-1',
      source: 'sample_data',
      title: 'Safe neighborhood with good lighting',
      body: 'This area is well-lit at night and has regular police patrols. I felt very safe walking around, even late in the evening.',
      url: '',
      username: 'SafetyReviewer',
      community_name: 'LocalInsights',
      created_at: new Date().toISOString(),
      is_safety_related: true,
      safety_score: 0.8,
      sentiment: 'positive',
      location_mentioned: 'neighborhood',
    ...
    {
      comment_id: 'sample-positive-2',
      source: 'sample_data',
      title: 'Friendly community',
      body: "The neighbors are very friendly and look out for each other. There is a strong sense of community which contributes to the overall safety.",
      url: '',
      username: 'LocalResident',
      community_name: 'NeighborhoodWatch',
      created_at: new Date().toISOString(),
      is_safety_related: true,
      safety_score: 0.85,
      sentiment: 'positive',
      location_mentioned: 'community',
      latitude: latitude + (Math.random() * 0.01 - 0.005),
      longitude: longitude + (Math.random() * 0.01 - 0.005),
      district: null,
      city: 'Los Angeles'
    ...
    {
      comment_id: 'sample-negative-1',
      source: 'sample_data',
      title: 'Watch out for parking issues',
      body: 'Parking can be difficult in this area, especially at night. Make sure to check street signs carefully to avoid getting towed.',
      url: '',
      username: 'ParkingExpert',
      community_name: 'CityLiving',
      created_at: new Date().toISOString(),
      is_safety_related: true,
      safety_score: 0.4,
      sentiment: 'negative',
      location_mentioned: 'street',
      latitude: latitude + (Math.random() * 0.01 - 0.005),
      longitude: longitude + (Math.random() * 0.01 - 0.005),
      district: null,
      city: 'Los Angeles'
    ...
    {
      comment_id: 'sample-negative-2',
      source: 'sample_data',
      title: 'Be cautious at night',
      body: 'While generally safe, I would recommend being aware of your surroundings when walking at night. Some areas are not as well-lit as others.',
      url: '',
      username: 'NightOwl',
      community_name: 'SafetyFirst',
      created_at: new Date().toISOString(),
      is_safety_related: true,
      safety_score: 0.35,
      sentiment: 'negative',
    ...
    {
      comment_id: 'sample-neutral-1',
      source: 'sample_data',
      title: 'Typical urban environment',
      body: 'This area has the usual safety considerations of any urban environment. Nothing particularly concerning, but standard city precautions apply.',
      url: '',
...
/**
 * Finds existing takeaways in the database or generates new ones from insights
 * @param latitude Latitude of the location
 * @param longitude Longitude of the location
 * @param radius Radius in kilometers
 * @returns SafetyTakeaways object
 */
export async function findOrGenerateTakeaways(
  ...
  try {
    ...
    if (!fetchError && existingTakeaways) {
      console.log('Found existing safety takeaways with ID:', existingTakeaways.id);
      return existingTakeaways;
    }

    // If no existing takeaways, fetch nearby insights to generate new ones
    const insights = await findNearbyInsights(latitude, longitude, radius);
    
    if (!insights || insights.length === 0) {
      console.log('No insights found for this location, returning fallback takeaways');
      return createFallbackTakeaway(latitude, longitude, radius);
    }

    // Generate takeaways from the insights
    console.log(`Generating takeaways from ${insights.length} insights`);
    const takeawaysContent = await generateTakeaways(insights);
    
    if (!takeawaysContent) {
      console.log('Failed to generate takeaways, returning fallback');
      return createFallbackTakeaway(latitude, longitude, radius);
    }
    ...
    const newTakeawayData: Omit<SafetyTakeaways, 'id' | 'created_at'> = {
      latitude,
      longitude,
      radius,
      positive_takeaway: takeawaysContent.positive,
      negative_takeaway: takeawaysContent.negative,
      neutral_takeaway: takeawaysContent.neutral,
    ...
    try {
      ...
      if (tableCheckError) {
        console.error('Error checking safety_takeaways table:', tableCheckError);
      ...
      console.log('Inserting new takeaway data:', newTakeawayData);
      ...
      if (insertError) {
        ...
        // Return the data even if we couldn't save it to the database
        return {
          id: `temp-${Date.now()}`,
          ...newTakeawayData,
          created_at: new Date().toISOString()
      ...
      console.log('Successfully stored takeaways in database with ID:', insertedTakeaway.id);
      return insertedTakeaway;
    } catch (dbError) {
      console.error('Database exception when inserting safety takeaways:', dbError);
      // Return the data even if we couldn't save it to the database
      return {
        id: `temp-${Date.now()}`,
        ...newTakeawayData,
...
/**
 * Creates default takeaway content when no insights are available or AI generation fails
 * @param latitude Latitude of the location
 * @param longitude Longitude of the location
 * @param radius Radius in kilometers
 * @returns SafetyTakeaways object with default content
 */
function createFallbackTakeaway(latitude: number, longitude: number, radius: number): SafetyTakeaways {
  const expiresAt = new Date();
  ...
  return {
    ...
    longitude,
    radius,
    positive_takeaway: "The area appears to have typical urban safety standards.",
    negative_takeaway: "No specific safety concerns were identified for this area.",
    neutral_takeaway: "Limited safety information is available for this specific location.",
    created_at: new Date().toISOString(),
    expires_at: expiresAt.toISOString()
  };
}
...
/**
 * Generates takeaways from a list of safety insights using Gemini AI
 * @param insights Array of safety insights
 * @returns Object containing positive, negative, and neutral takeaways
 */
async function generateTakeaways(insights: any[]): Promise<{ positive: string | null; negative: string | null; neutral: string | null } | null> {
  if (!insights || insights.length === 0) {
    console.log('No insights provided to generateTakeaways function');
    return null;
  }

  // Log insights sample for debugging
  console.log(`Generating takeaways from ${insights.length} insights. Sample:`, 
    insights.length > 0 ? JSON.stringify(insights[0], null, 2).substring(0, 200) + '...' : 'No insights');

  try {
    // Before passing to Gemini service, ensure insights have necessary fields
    const processedInsights = insights.map(insight => {
      // Ensure insight has all required fields
      return {
        ...insight,
        body: insight.body || insight.text || insight.title || insight.content || "No content available",
        sentiment: insight.sentiment || 'neutral',
        safety_score: insight.safety_score || 0.5
      };
    });
    
    // Use the centralized Gemini service to generate takeaways
    return await generateSafetyTakeaways(processedInsights);
  } catch (error) {
