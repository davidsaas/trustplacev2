...
export async function findOrGenerateTakeaways(latitude: number, longitude: number, radius: number = 2): Promise<SafetyTakeaways | null> {
  try {
    ...
    if (!fetchError && existingTakeaways) {
      console.log('Found existing takeaways for location');
      return existingTakeaways;
    }
    
    // If no valid takeaways found, fetch insights to generate new takeaways
    const insights = await findNearbyInsights(latitude, longitude, radius);
    
    if (insights.length === 0) {
      console.log('No insights found for location, cannot generate takeaways');
      return null;
    }
...
/**
 * Generate takeaways from safety insights using AI
 */
async function generateTakeaways(insights: any[]): Promise<{ positive: string | null; negative: string | null; neutral: string | null } | null> {
  ...
  const mixedInsights = insights.filter(i => i.sentiment === 'mixed' || i.sentiment === 'neutral');
  
  // Prepare insight texts for each sentiment
  const positiveTexts = positiveInsights.map(i => i.body).join('\n\n');
  const negativeTexts = negativeInsights.map(i => i.body).join('\n\n');
  const mixedTexts = mixedInsights.map(i => i.body).join('\n\n');
  
  // Initialize results
  let positiveTakeaway: string | null = null;
  let negativeTakeaway: string | null = null;
  let neutralTakeaway: string | null = null;
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      // Generate positive takeaway if we have positive insights
      if (positiveTexts && positiveTexts.length > 0) {
        const positivePrompt = `
          Analyze these positive comments about safety in this area and create a concise 1-2 sentence takeaway that summarizes the positive safety aspects.
          Focus on what makes the area safe according to these comments.
      ...
      // Generate negative takeaway if we have negative insights
      if (negativeTexts && negativeTexts.length > 0) {
        const negativePrompt = `
          Analyze these negative comments about safety concerns in this area and create a concise 1-2 sentence takeaway that summarizes the safety issues.
          Focus on specific concerns mentioned repeatedly.
          Return ONLY the takeaway text with no additional formatting, quotes, or prefixes.
      ...
      // Generate neutral/mixed takeaway if we have mixed insights
      if (mixedTexts && mixedTexts.length > 0) {
        const mixedPrompt = `
          Analyze these mixed or neutral comments about safety in this area and create a concise 1-2 sentence balanced takeaway.
          Focus on nuanced safety considerations that aren't clearly positive or negative.
          Return ONLY the takeaway text with no additional formatting, quotes, or prefixes.
          
          Comments:
          ${mixedTexts.substring(0, 1500)} // Limit text length
        `;
        
        const mixedResult = await model.generateContent(mixedPrompt);
        const mixedText = mixedResult.response.text().trim();
        
        // Remove any markdown formatting or quotes
        neutralTakeaway = mixedText
          .replace(/^["']|["']$/g, '') // Remove surrounding quotes
          .replace(/^#+\s+/g, '') // Remove markdown headers
          .replace(/^Takeaway:\s*/i, '') // Remove "Takeaway:" prefix
          .trim();
      ...
      if (positiveTakeaway || negativeTakeaway || neutralTakeaway) {
        return {
          positive: positiveTakeaway,
          negative: negativeTakeaway,
          neutral: neutralTakeaway
        };
      }
