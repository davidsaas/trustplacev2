...
export async function findNearbyInsights(latitude: number, longitude: number, radiusKm: number = 2) {
  try {
    // Get the existing safety insights from the database
    const { data, error } = await supabase.rpc('find_nearby_safety_insights', {
      lat: latitude,
      lng: longitude,
      radius_km: radiusKm
    });
    ...
    if (error) {
      console.error('Error finding nearby insights:', error);
      return null;
    }
    
    return data;
  } catch (error) {
    console.error('Failed to find nearby insights:', error);
    return null;
  }
}
...
/**
 * Find or generate takeaways for a location
 */
export async function findOrGenerateTakeaways(
  ...
  try {
    // Try to get existing takeaways first
    const existingTakeaways = await findExistingTakeaways(latitude, longitude, radius);
    if (existingTakeaways) {
      return existingTakeaways;
    }
    
    // If no existing takeaways, generate new ones
    // Get nearby insights
    const nearbyInsights = await findNearbyInsights(latitude, longitude, radius);
    
    // Generate takeaways using insights data
    const generatedTakeaways = await findOrGenerateLocationTakeaways(
      latitude, 
      longitude, 
      radius, 
      nearbyInsights || []
    ...
    // Convert to SafetyTakeaways with proper typing
    if (generatedTakeaways) {
      // Explicitly type the returned object
      const takeaways: SafetyTakeaways = {
        id: generatedTakeaways.id,
        latitude: generatedTakeaways.latitude,
        longitude: generatedTakeaways.longitude,
        radius: generatedTakeaways.radius,
        positive_takeaway: generatedTakeaways.positive_takeaway,
        negative_takeaway: generatedTakeaways.negative_takeaway,
        neutral_takeaway: null,  // Set explicitly to null
        created_at: generatedTakeaways.created_at,
        expires_at: generatedTakeaways.expires_at
      };
    ...
    console.error('Error finding or generating takeaways:', error);
    return null;
  }
}

// Helper function to find existing takeaways
async function findExistingTakeaways(
  latitude: number,
  longitude: number,
  radius: number
): Promise<SafetyTakeaways | null> {
  try {
    const { data, error } = await supabase.rpc('find_nearby_takeaways', {
      lat: latitude,
      lng: longitude,
      radius_km: radius
    });
    ...
    // Find the closest takeaway
    return sortByDistance(data, latitude, longitude)[0];
  } catch (error) {
    console.error('Error finding existing takeaways:', error);
    return null;
  }
}
...
/**
 * Get safety metrics for a given location
 * This combines the new safety metrics feature with the existing safety insights
 */
export async function getSafetyData(latitude: number, longitude: number, city: string = 'Los Angeles') {
  try {
    // Get district safety metrics
    const safetyMetrics = await getNearestDistrictSafetyMetrics(latitude, longitude, city);
    
    // Get existing safety insights from the old system (for backward compatibility)
    const safetyInsights = await findNearbyInsights(latitude, longitude);
    
    // Get safety takeaways (if available)
    const takeaways = await findOrGenerateTakeaways(latitude, longitude, 2);
