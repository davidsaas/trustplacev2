...
export async function findOrGenerateTakeaways(
  ...
  try {
    ...
    if (!fetchError && existingTakeaways) {
      console.log('Found existing safety takeaways');
      return existingTakeaways;
    }
    
    // If no existing takeaways, fetch insights and generate new ones
    ...
    try {
      ...
      console.error('Database error when inserting safety takeaways:', dbError);
      // Return the data even if we couldn't save it to the database
      return {
        id: `temp-${Date.now()}`,
        ...newTakeawayData,
