  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '42P01' || (error.message && error.message.includes('does not exist'))) {
        console.error('Table safety_insights does not exist:', error);
        return [];
      }
    ...
    if (!data || data.length === 0) {
      console.log('No safety insights found in the specified radius');
      return [];
    }
    
    console.log(`Found ${data.length} safety insights in radius of ${radiusKm}km`);
    ...
    console.error('Exception when fetching nearby insights:', error);
    return [];
  }
}
...
export async function findOrGenerateTakeaways(
  ...
  try {
    ...
    try {
      console.log('Checking for existing takeaways at:', { latitude, longitude, radius });
      ...
      if (fetchError) {
        if (fetchError.code !== 'PGRST116') { // Not found error is normal
          console.error('Error checking for existing takeaways:', fetchError);
        }
      } else if (existingTakeaways) {
        console.log('Found existing safety takeaways:', existingTakeaways.id);
        return existingTakeaways;
      }
    } catch (dbCheckError) {
      console.error('Exception checking for existing takeaways:', dbCheckError);
      // Continue to generate new takeaways
    }
    
    // If no existing takeaways, fetch nearby insights and generate new ones
    console.log('Finding nearby insights for:', { latitude, longitude, radius });
    const insights = await findNearbyInsights(latitude, longitude, radius);
    
    if (!insights || insights.length === 0) {
      console.log('No safety insights found for:', { latitude, longitude, radius });
      // Return a fallback takeaway
      return createFallbackTakeaway(latitude, longitude, radius);
    }
    
    console.log(`Found ${insights.length} insights, generating takeaways`);
    
    // Generate takeaways from insights
    const takeaways = await generateTakeaways(insights);
    
    if (!takeaways) {
      console.log('Failed to generate takeaways from insights');
      // Return a fallback takeaway
      return createFallbackTakeaway(latitude, longitude, radius);
    }
    ...
    console.log('Attempting to store takeaways in database');
    
    // Try to store in database
    try {
      // Insert the new takeaways directly
      ...
      if (insertError) {
        // If there's a table doesn't exist error, log it but return the data
        console.error('Error inserting safety takeaways:', insertError);
        
        // For PostgreSQL "relation does not exist" error
        if (insertError.code === '42P01' || 
            (insertError.message && insertError.message.includes('does not exist'))) {
          console.log('Table safety_takeaways does not exist, returning data without storing');
          return {
            id: `temp-${Date.now()}`,
            ...newTakeawayData,
    ...
    console.error('Error finding or generating safety takeaways:', error);
    return createFallbackTakeaway(latitude, longitude, radius);
  }
}

/**
 * Create a fallback safety takeaway
 */
...
async function generateTakeaways(insights: any[]): Promise<{ positive: string | null; negative: string | null; neutral: string | null } | null> {
  try {
    if (!insights || insights.length === 0) {
      console.log('No insights provided to generateTakeaways');
      return null;
    }

    // Count safety-related insights
    const safetyRelatedInsights = insights.filter(insight => 
      insight.is_safety_related === true && 
      insight.safety_score >= 0.6
    );

    if (safetyRelatedInsights.length < 2) {
      console.log('Not enough high-quality safety-related insights found:', safetyRelatedInsights.length);
      // We need at least a few good insights to generate quality takeaways
      return null;
    }

    // Extract key safety phrases from insights to force the model to use them
    const safetyPhrases = insights.flatMap(insight => {
      const text = insight.body.toLowerCase();
      const extractedPhrases = [];
      
      // Check for safety-related keywords and extract surrounding context
      const safetyTerms = [
        ...
        'secure', 'security', 'watch out', 'careful', 'caution',
        'warning', 'alert', 'protect', 'suspicious', 'incident'
      ];
      
      for (const term of safetyTerms) {
        if (text.includes(term)) {
          // Extract a phrase containing the term (the term and surrounding words)
          const words = text.split(' ');
          const termIndex = words.findIndex((word: string) => word.includes(term));
          if (termIndex >= 0) {
            // Get 5 words before and after the term
    ...
    if (safetyPhrases.length === 0) {
      console.log('No safety-related phrases found in insights');
      return null;
    }

    const prompt = `Analyze these SPECIFIC safety-related insights about a neighborhood and generate concrete, detailed safety takeaways directly based on these comments only.
    
    Create two distinct lists based ONLY on the information provided in these comments:
    1. "What's Good" - Specific safety positives explicitly mentioned in the insights
    2. "Watch Out For" - Specific safety concerns or warnings explicitly mentioned in the insights
    
    DO NOT make up generic safety advice. If the comments don't mention something specific, don't include it.
    Format each point as a separate sentence that directly references what was mentioned in the comments.
    
    Key safety phrases from the comments (use these exact phrases in your response):
    ${safetyPhrases.map(phrase => `- "${phrase}"`).join('\n')}
    ...
    {
      "positive": "Streets in the downtown area are described as \"well-lit at night\" by multiple residents. Several comments mention \"police regularly patrol the area\". Residents specifically note feeling \"safe walking around even late\".",
      "negative": "Multiple comments mention \"car break-ins near the park\". People specifically warn about \"sketchy characters hanging around the subway entrance at night\". Several residents note the \"back alley is poorly lit\".",
      "neutral": null
    }
    
    IMPORTANT: You MUST include at least one specific point in both positive and negative sections based on the comments. If there are no specific positive or negative points mentioned, say "Based on limited safety information, this area appears to have [positive/negative aspect]" but be specific about what insights led to this conclusion.`;

    console.log('Calling Gemini with extracted key phrases:', safetyPhrases.slice(0, 3));
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        try {
          ...
          // Only set to null if completely empty or explicitly stating nothing found
          if (!takeaways.positive || takeaways.positive.trim() === "" || 
              takeaways.positive.toLowerCase().includes("no specific safety positives") ||
              takeaways.positive.toLowerCase().includes("not mentioned")) {
            takeaways.positive = "Based on limited safety information, this area appears to have some positive aspects, though specific details are limited in available comments.";
          }
          
          if (!takeaways.negative || takeaways.negative.trim() === "" || 
              takeaways.negative.toLowerCase().includes("no specific safety concerns") || 
              takeaways.negative.toLowerCase().includes("not mentioned")) {
            takeaways.negative = "Based on limited safety information, there may be some areas to exercise caution, though specific details are limited in available comments.";
          }
          
          // Validate that the takeaways have the expected format
          if (typeof takeaways !== 'object' || 
              (takeaways.positive !== null && typeof takeaways.positive !== 'string') ||
              (takeaways.negative !== null && typeof takeaways.negative !== 'string')) {
            console.error('Invalid takeaways format:', takeaways);
            throw new Error('Invalid takeaways format');
          }
          
          return takeaways;
        } catch (e) {
          console.error(`Error parsing Gemini response (attempt ${attempts}/${maxAttempts}):`, e);
        ...
        console.error(`Error calling Gemini API (attempt ${attempts}/${maxAttempts}):`, error);
        if (attempts >= maxAttempts) {
          throw error;
        }
        // Wait before retrying
