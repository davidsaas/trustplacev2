...
async function generateTakeaways(insights: any[]): Promise<{ positive: string | null; negative: string | null; neutral: string | null } | null> {
  ...
  try {
    if (!process.env.GEMINI_API_KEY) {
      ...
      return {
        positive: "The area generally receives positive safety comments.",
        negative: "Exercise normal urban precautions in this area.",
    ...
    const insightData = insights.map(insight => {
      return {
        comment: insight.body || insight.title,
    ...
    console.log('Calling Gemini API with safety insights prompt...');
    const result = await model.generateContent(prompt);
    ...
    try {
      ...
      return {
        positive: formatTakeaway(takeaways.positive),
        negative: formatTakeaway(takeaways.negative),
        neutral: formatTakeaway(takeaways.neutral)
      };
    } catch (parseError) {
...
function extractTakeawaysFromText(text: string): { positive: string | null; negative: string | null; neutral: string | null } {
  try {
    ...
    return {
      positive: positiveMatch ? formatTakeaway(positiveMatch[1]) : "✓ The area appears generally safe",
      negative: negativeMatch ? formatTakeaway(negativeMatch[1]) : "⚠️ Exercise normal urban precautions",
      neutral: neutralMatch ? formatTakeaway(neutralMatch[1]) : "Safety standards appear typical for this type of location"
    };
  } catch (e) {
    return {
 ...
 */
function formatTakeaway(takeaway: string | null): string | null {
  if (!takeaway) return null;
  
  // Clean up the text
  let formatted = takeaway.trim();
  
  // Ensure each line starts with a checkmark or warning symbol if not already present
  if (!formatted.includes('✓') && !formatted.includes('⚠️')) {
    const lines = formatted.split('\n');
    if (formatted.toLowerCase().includes('concern') || 
        formatted.toLowerCase().includes('avoid') || 
        formatted.toLowerCase().includes('danger')) {
      // Likely negative points
      formatted = lines.map(line => line.trim().startsWith('⚠️') ? line : `⚠️ ${line}`).join('\n');
    } else {
      // Likely positive points
      formatted = lines.map(line => line.trim().startsWith('✓') ? line : `✓ ${line}`).join('\n');
    }
  }
