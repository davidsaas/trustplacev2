...
// Handle environment variables during build time vs runtime
let supabase: any;
let genAI: any;
let model: any;

// Only initialize clients if we're not in a build environment
if (process.env.NODE_ENV !== 'production' || process.env.VERCEL_ENV !== 'build') {
  ...
  // During build time, create placeholders
  supabase = {};
  genAI = {};
  model = { generateContent: async () => ({ response: { text: () => 'Placeholder during build' } }) };
}
...
export async function findNearbyInsights(latitude: number, longitude: number, radiusKm: number = 2) {
  ...
  console.log('No insights found from any source, returning empty array');
  return [];
}
