...
export async function findNearbyInsights(latitude: number, longitude: number, radiusKm: number = 2) {
  ...
  // First try to get safety-related insights
  ...
  console.log(`Found ${safetyData.length} safety-related insights`);
  
  // If we didn't find enough safety insights, also get insights without the safety flag
  if (safetyData.length < 5) {
    console.log('Not enough safety insights, fetching additional insights');
    ...
    if (additionalError) {
      ...
      console.log(`Found ${additionalData.length} additional insights`);
      
      // Combine the datasets, removing duplicates
      const combinedData = [...safetyData];
      const existingIds = new Set(safetyData.map(item => item.comment_id));
      
      for (const item of additionalData) {
        if (!existingIds.has(item.comment_id)) {
          combinedData.push(item);
          existingIds.add(item.comment_id);
        }
      }
      
      // If we found additional data, use the combined dataset
      if (combinedData.length > safetyData.length) {
        console.log(`Using combined dataset with ${combinedData.length} insights`);
  ...
  // If no additional insights were found or needed, use the safety-related insights
  // Sort by distance (Euclidean distance as a simple approximation)
