...
export const getNearestDistrictSafetyMetrics = async (
  ...
  try {
    console.log(`Finding nearest district for coordinates: lat=${latitude}, lng=${longitude}`);
    ...
    if (error) {
      console.error('Error in RPC find_la_district:', error);
      return null;
    }
    
    if (!data || !data.district) {
      console.log('No district found for coordinates:', { latitude, longitude });
      // Fallback to downtown LA if no district found (this is a developer convenience)
      const downtownDistrict = 'Downtown';
      console.log(`Using fallback district: ${downtownDistrict}`);
      return await getDistrictSafetyMetrics(downtownDistrict, city);
    }
    
    console.log(`Found district: ${data.district}, distance: ${data.distance_km.toFixed(2)}km`);

    // Now fetch the safety metrics for this district
    const metrics = await getDistrictSafetyMetrics(data.district, city);
    
    if (!metrics) {
      console.log(`No safety metrics found for district: ${data.district}`);
    }
