...
export function findSaferAlternatives(
  ...
  // Find any listings in the same city with a better safety score
  const betterListings = allListings
    .filter(listing => {
      ...
      return (
        ...
        // Better safety score (even slightly better)
        calculateSafetyScore(listing) > currentSafetyScore
      );
    });
    
  // If we have at least 3 listings that match our criteria, sort and return them
  if (betterListings.length >= limit) {
    return betterListings
      .sort((a, b) => calculateSafetyScore(b) - calculateSafetyScore(a))
      .slice(0, limit);
  }
  
  // If we don't have enough listings, relax the city constraint
  return allListings
    .filter(listing => 
      // Exclude current listing
      listing.id !== currentListing.id &&
