...
export async function generateTakeaways(
  ...
  try {
    ...
    console.log('Content for analysis:', contentText);
...
- If safety information is limited, make reasonable inferences based on context`;
    ...
    try {
      ...
      console.log('Formatted takeaways:', formattedTakeaways);
      ...
      console.log('Extracted takeaways via regex:', extractedTakeaways);
      
      return extractedTakeaways;
    }
  } catch (error) {
...
function formatTakeaway(takeaway: string | null, type: 'positive' | 'negative'): string | null {
  if (!takeaway) {
    console.log(`${type} takeaway is null, returning null`);
    return null;
  }
  
  // Clean up the text
  let formatted = takeaway.trim();
  console.log(`Formatting ${type} takeaway:`, formatted);
  ...
  if (formatted.length === 0) {
    console.log(`${type} takeaway is empty after formatting, returning null`);
    return null;
  }
  
  console.log(`Final formatted ${type} takeaway:`, formatted);
  return formatted;
}
