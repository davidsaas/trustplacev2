...
- If safety information is limited, it's better to have fewer specific points than many generic points
...
function formatTakeaway(takeaway: string | null, type: 'positive' | 'negative'): string | null {
  ...
  // List of generic tips to filter out
  const genericTips = [
  ...
  // Problematic patterns to filter
  ...
  formatted = lines
    .map(line => {
      ...
      // Skip problematic or incomplete lines
      if (problematicPatterns.some(pattern => line.toLowerCase().includes(pattern.toLowerCase()))) {
        return '';
      }
      
      // Skip generic safety tips
      if (genericTips.some(tip => line.toLowerCase().includes(tip.toLowerCase()))) {
        return '';
      }
...
export async function generateVideoTakeaways(
  ...
  try {
    ...
    const newTakeawayData: Omit<VideoTakeaways, 'id' | 'created_at'> = {
      ...
      positive_takeaway: takeaways.positive_takeaway,
      negative_takeaway: takeaways.negative_takeaway,
      neutral_takeaway: null, // Default to null as it's not generated by default
      summary: summary,
      title: content.title,
...
export async function findOrGenerateVideoTakeaways(
  ...
  try {
    // First check if this video already exists for this location
    ...
    if (!checkError && existingVideo) {
      console.log(`Video ${videoId} already exists for location ${locationId}`);
      
      // Check for existing takeaways
      const { data: existingTakeaways, error: fetchError } = await supabase
        .from('video_takeaways')
        .select('*')
        .eq('video_id', videoId)
    ...
    // If video doesn't exist for this location or doesn't have takeaways, generate them
