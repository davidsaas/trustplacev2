...
# Map LAPD area codes to district names
LAPD_AREA_TO_DISTRICT = {
    '1': 'Central',
    '2': 'Rampart',
    '3': 'Southwest',
    '4': 'Hollywood',
...
def process_crime_data(crime_data):
    ...
    # Convert area code to district name
    df['district'] = df['area'].map(LAPD_AREA_TO_DISTRICT)
    
    # Process by district
    district_metrics = {}
    
    for district, records in df.groupby('district'):
        # Skip if district is None or not mapped
        if not district:
            continue
...
def process_safety_metrics(city='Los Angeles'):
    ...
    if success:
        ...
        logger.error(f"Failed to update database for {city}")
