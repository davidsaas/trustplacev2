      ...
      const addLocationMarker = (location: { location: string; safetyScore: number; coordinates: [number, number]; url?: string }, isMain: boolean = false) => {
        ...
        markerElement.innerHTML = `
          <div style="
            ...
            background: ${isMain ? getSafetyColor(location.safetyScore) : `${getSafetyColor(location.safetyScore)}CC`};
            border-radius: 50%;
        ...
        if (location.url && onLocationClick) {
          ...
          if (markerDiv) {
            markerElement.addEventListener('mouseenter', () => {
              ...
              if (!isMain) {
                markerDiv.style.background = getSafetyColor(location.safetyScore); // Full opacity on hover
              }
            });
            markerElement.addEventListener('mouseleave', () => {
              markerDiv.style.transform = 'scale(1)';
              if (!isMain) {
                markerDiv.style.background = `${getSafetyColor(location.safetyScore)}CC`; // Back to semi-transparent
              }
            });
