...
export default function VideoTakeaways({ videos, isLoading = false, locationName }: VideoTakeawaysProps) {
  ...
  const preprocessTakeaway = (text: string | null): string[] => {
    ...
    return cleanText.split('\n')
      .map(line => line.trim())
      .filter(line => line.length > 0 && line !== "null" && !line.match(/^[✓⚠️]\s*null$/));
  };

  // Generate fallback positive takeaways when none are found
  const generateFallbackPositiveTakeaways = (location: string = 'this location'): string[] => {
    // Create location-specific fallback takeaways
    return [
      `Most tourist attractions in ${location} have standard safety measures in place`,
      `Like many popular destinations, ${location} is generally safe for tourists who take basic precautions`,
      `The video provides safety information which helps visitors enjoy ${location} responsibly`
    ];
  };
  ...
  return (
    <div className="space-y-6">
      {videos.map((video) => {
        ...
        // Generate fallback positive takeaways if there are negative takeaways but no positive ones
        const useFallbackPositives = !hasPositiveTakeaways && hasNegativeTakeaways;
        const fallbackPositives = useFallbackPositives ? generateFallbackPositiveTakeaways(locationName) : [];
        
        // Determine if we have any takeaways to show
        const hasTakeaways = hasPositiveTakeaways || hasNegativeTakeaways || useFallbackPositives;
        
        // Default tab to show
        const defaultTab = (hasPositiveTakeaways || useFallbackPositives) ? "positive" : 
                ...
                <Tabs defaultValue={defaultTab} className="w-full">
                  <TabsList className="w-full grid grid-cols-2 bg-slate-100 mb-4">
                    <TabsTrigger 
                      ...
                      disabled={!hasPositiveTakeaways && !useFallbackPositives}
                    >
                  ...
                  <TabsContent value="positive" className="space-y-3">
                    ...
                    ) : useFallbackPositives ? (
                      <div className="space-y-3">
                        {fallbackPositives.map((point, index) => (
                          <div key={index} className="flex items-start gap-2">
                            <CheckCircle className="h-5 w-5 text-emerald-600 mt-0.5 flex-shrink-0" />
                            <p className="text-emerald-700">{point}</p>
                          </div>
                        ))}
                        {/* Note about fallback content */}
                        <p className="text-xs text-gray-500 italic mt-2 pt-2 border-t">
                          This video focuses on safety concerns. These are general safety observations.
                        </p>
                      </div>
                    ) : (
