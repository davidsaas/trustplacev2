...
export default function MapView({ location, safetyScore, coordinates, onLoad }: MapViewProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Once the map is loaded, add the safety radius and call onLoad
      map.current.on('load', () => {
        if (!map.current) return;
        ...
        // Call onLoad callback when map is fully initialized
        onLoad?.();
      });
    } catch (error) {
      console.error('Error initializing Mapbox map:', error);
      // Still call onLoad even if there's an error to prevent infinite loading state
      onLoad?.();
    }
  ...
  }, [coordinates, location, safetyScore, onLoad]); // Added onLoad to dependencies
