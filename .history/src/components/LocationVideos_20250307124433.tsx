...
export default function LocationVideos({ locationId, locationName, isLoading = false }: LocationVideosProps) {
  ...
  useEffect(() => {
    ...
    const processTakeaways = async (videos: LocationVideo[]) => {
      ...
      try {
        // Filter out duplicate videos (same video_id)
        const uniqueVideos = videos.filter((video, index, self) => 
          index === self.findIndex(v => v.video_id === video.video_id)
        );
        
        // For a real implementation, we would call the findOrGenerateVideoTakeaways function 
        // for each video to get proper takeaways from the backend
        // In this demo, we're simulating the takeaways
        
        const takeaways = uniqueVideos.map(video => ({
          id: video.id,
