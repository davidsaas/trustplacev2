...
export default function ReviewTakeaways({ reviews, listingId, isLoading }: ReviewTakeawaysProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Safety-Related Reviews Section */}
      {safetyReviews.length > 0 && (
        <div className="mt-8">
          <h4 className="text-lg font-semibold text-gray-900 mb-4">Safety-Related Reviews</h4>
          <div className="space-y-4">
            {safetyReviews.map((safetyReview) => (
              <Card key={safetyReview.review.id} className="p-4 hover:shadow-md transition-shadow">
                <div className="flex items-start gap-4">
                  <Avatar className="h-10 w-10">
                    <AvatarImage src={safetyReview.review.authorImage} />
                    <AvatarFallback>{safetyReview.review.author.slice(0, 2).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-medium text-gray-900">{safetyReview.review.author}</span>
                      <span className="text-sm text-gray-500">{formatDate(safetyReview.review.date)}</span>
                    </div>
                    ...
                    <p className="text-gray-700 mb-2">{safetyReview.review.text}</p>
                    <div className={`text-sm p-2 rounded-lg ${
                      safetyReview.sentiment === 'positive'
                        ? 'bg-emerald-50 text-emerald-700'
                        : safetyReview.sentiment === 'negative'
                        ? 'bg-rose-50 text-rose-700'
                        : 'bg-amber-50 text-amber-700'
                    }`}>
                      <strong>Safety Context:</strong> {safetyReview.safetyContext}
                    </div>
                  </div>
                </div>
              </Card>
