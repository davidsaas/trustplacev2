...
export default function LocationVideos({ locationId, locationName, isLoading = false }: LocationVideosProps) {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      try {
        setLoading(true);
        console.log(`Fetching videos for location: ${locationName} (ID: ${locationId})`);
        ...
        if (data.success && data.videos) {
          ...
          console.log(`Received ${sortedVideos.length} videos`);
          setVideos(sortedVideos);
        } else {
          console.error('No videos in response or success is false:', data);
