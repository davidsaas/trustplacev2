...
export async function GET(request: Request) {
  try {
    // Initialize the Supabase client with server-side auth
    ...
    const processedVideos = await Promise.all(
      youtubeVideos.map(async (video) => {
        try {
          ...
          if (!storedVideo) {
            console.error('Failed to store video:', videoId);
          }
    ...
    const successfulVideos = processedVideos.filter(video => video !== null);
