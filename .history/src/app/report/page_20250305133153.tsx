...
import { ApifyListing, fetchLAListings, calculateSafetyScore, extractSafetyReviews, findSaferAlternatives } from "@/lib/apify";
...
export default function ReportPage() {
  ...
  const [safetyReviews, setSafetyReviews] = useState<any[]>([]);
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      try {
        ...
        // Find the listing that matches the URL
        const matchingListing = listings.find(l => l.url === url);
        ...
        <div className="md:col-span-2 space-y-6">
          <Card>
            <div className="rounded-t-lg overflow-hidden max-h-64">
              <img
                src={listing.photos?.[0]?.large || '/images/placeholder.jpg'}
            ...
            <CardContent className="p-6">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-2">${listing.price.amount} per night</p>
                </div>
              ...
              {alternatives.map((alt) => (
                ...
                >
                  <div className="h-16 w-16 rounded-md overflow-hidden flex-shrink-0">
                    <img
                      src={alt.photos?.[0]?.large || '/images/placeholder.jpg'}
                      alt={alt.photos?.[0]?.caption || alt.title}
                      className="h-full w-full object-cover"
                    />
                  </div>
                  <div className="flex-1">
                    <p className="font-medium line-clamp-1">{alt.title}</p>
                    <p className="text-sm">${alt.price.amount}/night</p>
