...
export default function ReportPage() {
  ...
  // Update the ref when activeSection changes
  useEffect(() => {
    activeSectionRef.current = activeSection;
  }, [activeSection]);
  
  // Memoize the sections to avoid recreation on render
  ...
  // Improved Intersection Observer to minimize re-renders
  useEffect(() => {
    if (isLoading) return;
    ...
    const updateActiveSection = (newSection: string) => {
      if (newSection !== activeSectionRef.current) {
        // Only schedule a state update if really needed
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-indigo-50 pb-12">
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="block md:hidden mb-6">
          <div className="h-[300px] w-full rounded-xl overflow-hidden shadow-md" style={{ minHeight: '300px' }}>
            {listing && listing.location.coordinates && (
              <MapView
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="hidden md:block w-full md:w-2/3">
            <div className="sticky top-24 h-[calc(100vh-6rem)] rounded-xl overflow-hidden shadow-md" style={{ minHeight: '400px' }}>
              {listing && listing.location.coordinates && (
                <MapView
                  location={`${listing.location.city}, ${listing.location.state}`}
                  safetyScore={safetyMetrics?.overallSafety || safetyScore}
