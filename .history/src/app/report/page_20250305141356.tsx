...
export default function ReportPage() {
  ...
  // Helper function to get an image URL from a listing
  const getImageUrl = (listing: ApifyListing): string => {
    // Try various possible image sources
    if (listing.pictureUrl) return listing.pictureUrl;
    if (listing.thumbnail) return listing.thumbnail;
    if (listing.image) return listing.image;
    if (listing.imagePath) return listing.imagePath;
    
    // Check for author image as last resort
    if (listing.reviews?.details?.reviews?.[0]?.author?.pictureUrl) {
      return listing.reviews.details.reviews[0].author.pictureUrl;
    }
    
    // Default fallback
    return '/images/placeholder-home.jpg';
  };
